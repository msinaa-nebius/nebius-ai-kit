...
Read the role map first if one exists. Look in this order and take the first file
whose header says it was generated by nebius-ai-kit: `~/nebius-ai/ROLE-MAP.md`;
then `nebius-ai/ROLE-MAP.md` and `ROLE-MAP.md` in the current workspace
(environments that cannot write to the home folder save there); then any path the
person's own global instructions name. Nowhere to be found but they say they have
one — ask for the path once. It tells you their vocabulary,
