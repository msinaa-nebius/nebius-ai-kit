...
3. Only when all three files verified: the destinations are the skills folders
   ...
   product I use. Check writability of each destination (create and delete a
   temp file) before asking me anything about it. Then check whether any of
...
4. Re-read the three files at every destination that now holds them — written
   ...
   as kept, not checked).
5. Only if all three files verified at at least one destination: if
   ~/nebius-ai/ROLE-MAP.md exists AND its header says it was generated by
   nebius-ai-kit, say the kit is installed/updated and that saying "rebuild my
   role map" refreshes it — do not rerun onboarding. (A file there without
   that header is the user's own — leave it alone and mention it.) Otherwise
...
asks two kinds of question: the confirmation in step 0, and — if something is
already installed — whether to overwrite it.
...
folders named in its installed MANIFEST (today: nebius-setup, nebius-role-map and
nebius-ask) from ~/.claude/skills and ~/.codex/skills; if ~/nebius-ai/ exists, remind me what it holds and ask whether to
...
5. On a new release: new tag (`v1.1`), never move an existing tag. Optionally,
